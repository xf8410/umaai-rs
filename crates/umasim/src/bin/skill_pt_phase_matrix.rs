...
const BASE_SEED:u64=61444; const UMA:u32=102601;
...
struct PhaseTrainer{years:[LocalRamenTrainer;3],last:Mutex<Option<usize>>}
impl PhaseTrainer{
 fn new(pt:[u32;3],sac:u32,extra:&str)->Result<Self>{
  ...
  Ok(Self{years:[make(pt[0])?,make(pt[1])?,make(pt[2])?],last:Mutex::new(None)})
 }
 fn year(g:&RamenGame)->usize{if g.turn()<24{0}else if g.turn()<48{1}else{2}}
}
impl Trainer<RamenGame> for PhaseTrainer{
 fn select_action(&self,g:&RamenGame,a:&[RamenAction],r:&mut StdRng)->Result<usize>{let y=Self::year(g);*self.last.lock().unwrap()=Some(y);self.years[y].select_action(g,a,r)}
 fn select_choice(&self,g:&RamenGame,c:&[Vec<EventChoice>],r:&mut StdRng)->Result<usize>{let y=Self::year(g);*self.last.lock().unwrap()=Some(y);self.years[y].select_choice(g,c,r)}
 fn select_event_choice(&self,g:&RamenGame,e:&EventData,c:&[Vec<EventChoice>],r:&mut StdRng)->Result<usize>{let y=Self::year(g);*self.last.lock().unwrap()=Some(y);self.years[y].select_event_choice(g,e,c,r)}
 fn last_breakdown(&self)->Option<String>{let y=(*self.last.lock().ok()?)?;self.years[y].last_breakdown()}
}
