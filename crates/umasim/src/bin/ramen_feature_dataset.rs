...
use serde::Serialize;
use umasim::{
    game::{Game, InheritInfo, Person, ramen::{Operation, RamenAction, RamenGame, RamenStage}},
...
fn stage_index(stage: &RamenStage) -> usize {
    match stage {
        RamenStage::Begin => 0, RamenStage::Distribute => 1, RamenStage::RamenSelect => 2,
        RamenStage::SpecialSelect => 3, RamenStage::Train => 4, RamenStage::AfterTrain => 5,
        RamenStage::NextTurn => 6, RamenStage::RegionSelect => 7,
        RamenStage::SuperRamenSelect => 8, RamenStage::Settlement => 9,
    }
}
...
fn action_features(game: &RamenGame, action: &RamenAction) -> Vec<f32> {
    ...
    match action.operation {
        ...
        Operation::RegionSelect(regions) => {
            f[10] = 1.0;
            for r in regions { if r < 20 { f[31 + r] = 1.0; } }
        }
        Operation::StageOnly => f[11] = 1.0,
    }
    if let Some(r) = action.ramen {
        f[12] = 1.0;
        if r < 20 { f[13 + r.min(17)] = 1.0; }
    ...
    if let Some(t) = action.special_targets {
        for i in 0..3 { f[55 + i] = t[i] as f32 / 4.0; }
    }
    let _ = game;
    f
}
...
fn state_s(game: &RamenGame) -> Result<Vec<f32>> {
    ...
    for i in 0..10 { f.push((stage_index(&game.stage) == i) as u8 as f32); }
    for &v in &game.uma.five_status { f.push(v as f32 / 3000.0); }
    for &v in &game.uma.five_status_limit { f.push(v as f32 / 3000.0); }
    for i in 0..5 { f.push((game.uma.five_status_limit[i] - game.uma.five_status[i]).max(0) as f32 / 3000.0); }
    f.extend([
        game.uma.vital as f32 / 100.0, game.uma.max_vital as f32 / 120.0,
        game.uma.motivation as f32 / 5.0, game.uma.skill_pt as f32 / 15000.0,
        game.uma.total_hints as f32 / 100.0, game.ramen.scenario_pt as f32 / 6000.0,
        game.ramen.special_feeling as f32 / 4.0, game.ramen.eat_count as f32 / 8.0,
        game.ramen.train_level_bonus as f32 / 5.0, game.ramen.rmj_results.len() as f32 / 3.0,
    ]);
    for &v in &game.ramen.feeling_stock { f.push(v as f32 / 10.0); }
    for &v in &game.ramen.feeling_slot { f.push(v as f32 / 7.0); }
    for &v in &game.card_type_count[..5] { f.push(v as f32 / 3.0); }
    for train in 0..5 {
        let buffs = game.calc_training_buff(train)?;
        let value = game.calc_training_value(&buffs, train)?;
        for &v in &value.status_pt { f.push(v as f32 / 200.0); }
        ...
        let hints = game.distribution().get(train).into_iter().flatten().filter(|&&p| p >= 0 && game.persons()[p as usize].hint()).count();
...
fn state_m(game: &RamenGame, mut f: Vec<f32>, with_id: bool) -> Vec<f32> {
    for (idx, card) in game.deck().iter().enumerate() {
        for t in 0..6 { f.push((card.card_type.clamp(0, 5) as usize == t) as u8 as f32); }
        ...
        for train in 0..5 { f.push(game.distribution().get(train).is_some_and(|d| d.contains(&(idx as i32))) as u8 as f32); }
        if with_id { f.push(card.card_id as f32 / 40000.0); f.push(card.data.chara_id as f32 / 2000.0); }
    }
    f
}
...
fn state_l(game: &RamenGame, mut f: Vec<f32>) -> Vec<f32> {
    for train in 0..5 {
        for slot in 0..5 {
            ...
            f.push(person.filter(|p| *p >= 0).map(|p| game.persons()[p as usize].friendship() as f32 / 100.0).unwrap_or(0.0));
            f.push(person.filter(|p| *p >= 0).map(|p| game.persons()[p as usize].hint() as u8 as f32).unwrap_or(0.0));
        }
    }
    f.extend([
        game.current_effect.xunlian as f32 / 100.0, game.current_effect.youqing as f32 / 100.0,
        game.current_effect.pt_bonus as f32 / 100.0, game.current_effect.fail_rate_drop / 100.0,
        game.current_effect.friendship as f32 / 20.0, game.current_effect.deyilv as f32 / 200.0,
        game.current_effect.hint as f32 / 100.0, game.current_effect.clone as f32 / 6.0,
        game.current_effect.hint_special as u8 as f32, game.deck_can_split as u8 as f32,
    ]);
    for r in 0..20 { f.push(game.ramen.selected_regions.contains(&r) as u8 as f32); }
    f
}
...
fn main() -> Result<()> {
    ...
    let search: FlatSearch<RamenGame> = FlatSearch::new(
        SearchConfig::default().with_search_n(rollouts).with_ucb(false).with_radical_factor_max(0.0),
    );
    let mut out = File::create("ramen-feature-dataset.jsonl")?;
    for index in start..start + count {
        let deck_idx = index as usize % DECKS.len();
        let spec = SampleSpec {
            index, uma: UMA, deck: DECKS[deck_idx], shape: BUILD_NAMES[deck_idx], inherit: INHERIT,
            truncate_turn: ((index.wrapping_mul(37).wrapping_add(11)) % 72) as i32,
            seed: BASE_SEED ^ index.wrapping_mul(0x9e37_79b9_7f4a_7c15), epsilon: 0.15, min_actions: 2,
        };
        let SampleOutcome::Captured(pos) = sample_from_spec(spec)? else { continue; };
        ...
        for (action_index, action) in pos.actions.iter().enumerate() {
            ...
            let append = |mut x: Vec<f32>| { x.extend_from_slice(&af); x };
            let ar = &result.action_results[action_index].0;
            let row = Row {
                group: index, build: BUILD_NAMES[deck_idx], turn: pos.turn, stage: format!("{:?}", pos.stage),
                action_index, action: action.to_string(), teacher_mean: ar.mean() as f32,
                teacher_stdev: ar.stdev() as f32, s: append(s0.clone()), m: append(m0.clone()),
                mid: append(mid0.clone()), l: append(l0.clone()),
            };
            serde_json::to_writer(&mut out, &row)?;
            writeln!(out)?;
        }
    }
    eprintln!("schema dims: S={} M={} MID={} L={} (含动作58维)", state_s(&RamenGame::newgame(UMA, &DECKS[0], INHERIT)?)?.len()+58, 0, 0, 0);
