...
impl ManualTrainer {
    pub fn new() -> Self {
        Self { mock_inputs: Rc::new(RefCell::new(VecDeque::new())), fallback: FallbackMode::Interactive }
    }

    pub fn with_mock_inputs(inputs: Vec<String>) -> Self {
        Self { mock_inputs: Rc::new(RefCell::new(inputs.into_iter().collect())), fallback: FallbackMode::PickFirst }
    }
...
impl<G: Game> Trainer<G> for ManualTrainer {
    fn select_action(&self, _game: &G, actions: &[<G as Game>::Action], _rng: &mut StdRng) -> Result<usize> {
        if let Some(input) = self.pop_mock_input() {
            return actions.iter().position(|x| x.to_string() == input)
        ...
        match self.fallback {
            ...
            FallbackMode::Interactive => {
                ...
                actions.iter().position(|x| *x == selected)
                    .ok_or_else(|| anyhow::anyhow!("未找到该动作: {selected}"))
            }
            #[cfg(not(feature = "cli"))]
            FallbackMode::Interactive => Err(anyhow::anyhow!("ManualTrainer::Interactive 需要 cli feature"))
        }
    }

    fn select_choice(&self, _game: &G, choices: &[Vec<EventChoice>], _rng: &mut StdRng) -> Result<usize> {
        let explain: Vec<String> = choices.iter()
            .map(|x| x.iter().map(|y| y.explain()).collect::<Vec<_>>().join(" | ")).collect();
        if let Some(input) = self.pop_mock_input() {
            return explain.iter().position(|x| x == &input)
        ...
        match self.fallback {
            ...
            FallbackMode::Interactive => {
                ...
                explain.iter().position(|x| x == &selected)
