...
impl Default for LocalRamenConfig {
    fn default() -> Self {
        Self {
            ...
            low_friend_bond_value: 35.0, active_friend_value: 8.0, high_fail_penalty: 0.0,
            feeling_overflow_threshold: 8, overflow_value: 8.0, max_base_score_sacrifice: 140.0,
            status_reserve_max: 0.0, dynamic_vital: false, probabilistic_hint: false, expected_fail: false
        }
    }
}
...
impl LocalRamenTrainer {
    ...
    pub fn matrix_variant(name: &str) -> Result<Self> {
        ...
        for token in name.split('-') {
            // Exact structural tokens must be handled before prefix-valued tokens. In particular,
            // `failmodel` also starts with `fail`; parsing the prefix first tried to parse "model" as f32.
            if token == "failmodel" {
    ...
    fn dynamic_vital_factor(turn:i32)->f32 { if turn>=72 {0.25} else {3.5+(turn as f32/72.0)*2.0} }
    ...
    fn decide_train(&self, game:&RamenGame, actions:&[RamenAction])->Result<(usize,Vec<RamenPolicyOutput>)>{
        ...
        let base_best=Self::choose(&outputs); let phase=Self::phase_scale(game.turn());
        for (action,output) in actions.iter().zip(outputs.iter_mut()) {
            let Operation::Train(tt)=action.operation else {continue}; let training=tt as usize;
            let buffs=game.calc_training_buff(training)?; let value=game.calc_training_value(&buffs,training)?;
            ...
            for person_index in people {
                ...
                match person.person_type(){
                    PersonType::ScenarioCard=>long_term+=match game.friend.out_state{FriendOutState::UnClicked=>self.config.first_friend_click_value,_ if person.friendship()<60=>self.config.low_friend_bond_value*phase,_=>self.config.active_friend_value},
                    PersonType::Card if person.friendship()<80=>{let mut bond=if game.uma.flags.aijiao{9.0}else{7.0};if person.hint(){bond+=5.0*hint_prob;}bond=bond.min((80-person.friendship()) as f32);long_term+=bond*self.config.early_bond_value*phase;if person.hint(){long_term+=self.config.hint_bonus*hint_prob;}},
                    PersonType::Card if person.hint()=>long_term+=self.config.hint_bonus*hint_prob,_=>{}
                }
            }
            output.score+=long_term; output.add("local_long_term",long_term);
            let reserve=-self.reserve_penalty(game,&value.status_pt); output.score+=reserve; output.add("future_status_reserve",reserve);
            if self.config.dynamic_vital {let cost=(-value.vital).max(0) as f32;let adjust=-cost*(Self::dynamic_vital_factor(game.turn())-self.policy.config.train_vital_value);output.score+=adjust;output.add("dynamic_vital",adjust);}
            let failure=game.calc_training_failure_rate(&buffs,training);
            if self.config.expected_fail&&failure>0.0 {let p=failure/100.0;let big_p=if failure>=20.0{p}else{0.0};let extra=-p*(150.0+big_p*350.0-self.policy.config.failure_penalty);output.score+=extra;output.add("expected_fail_layers",extra);}
            else if failure>15.0&&self.config.high_fail_penalty>0.0 {let penalty=-((failure-15.0)/85.0).clamp(0.0,1.0)*self.config.high_fail_penalty;output.score+=penalty;output.add("local_high_fail_tail",penalty);}
        }
        let local_best=Self::choose(&outputs); let sacrifice=base_scores[base_best]-base_scores[local_best];
    ...
    fn decide_ramen(&self,game:&RamenGame,actions:&[RamenAction])->Result<(usize,Vec<RamenPolicyOutput>)>{
        let (_,mut outputs)=self.policy.decide_ramen(game,actions)?;let risk=(game.ramen.feeling_stock.iter().sum::<i32>()-self.config.feeling_overflow_threshold).max(0) as f32;
        for (a,o) in actions.iter().zip(outputs.iter_mut()){if a.ramen.is_some(){let b=risk*self.config.overflow_value;o.score+=b;o.add("local_stock_pressure",b);}} Ok((Self::choose(&outputs),outputs))
    }
}
impl Trainer<RamenGame> for LocalRamenTrainer {
    fn select_action(&self,game:&RamenGame,actions:&[RamenAction],_rng:&mut StdRng)->Result<usize>{if actions.len()<=1{return Ok(0);}let(choice,outputs)=match game.stage{RamenStage::Train=>self.decide_train(game,actions)?,RamenStage::RamenSelect=>self.decide_ramen(game,actions)?,RamenStage::SpecialSelect=>self.policy.decide_special(game,actions)?,RamenStage::RegionSelect=>{let y=match game.turn(){2=>0,23=>1,47=>2,_=>0};self.policy.decide_region(game,y,actions)?},_=>(0,Vec::new())};self.stash(&outputs);Ok(choice)}
...
mod tests {
    use super::*;
    #[test]
    fn matrix_variant_parses_failmodel_before_numeric_fail() {
        let plain = LocalRamenTrainer::matrix_variant("pt16-sac140-long-fail0-failmodel");
        assert!(plain.is_ok());
        let combined = LocalRamenTrainer::matrix_variant("pt64-sac200-long-fail0-vital-failmodel");
